...
public class DwellerChaseGoal extends Goal {
    ...
    private double movementSpeed = CaveNoise.CONFIG.MOVEMENT_SPEED();
    ...
    public DwellerChaseGoal(CaveDwellerEntity caveDweller, boolean followTargetEvenIfNotSeen, float ticksUntilCanAttack) {
        ...
        this.nodePos = null;
        this.ticksUntilLeave = Utils.secondsToTicks(CaveNoise.CONFIG.TIME_UNTIL_LEAVE_CHASE());
    ...
    public void tick() {
        ...
        if (!squeezing) {
            ...
            this.movementSpeed = (CaveNoise.CONFIG.MOVEMENT_SPEED());

        } else {
            this.movementSpeed = (CaveNoise.CONFIG.MOVEMENT_SPEED() * 1.2);
        }
        ...
        if (Utils.isValidPlayer(target)) {
            if (CaveNoise.CONFIG.GIVE_DARKNESS()) {
    ...
    public void aggroTick() {
        ...
        if (this.caveDweller.getNavigation().getPath() != null) {
            ...
            if (tempClimbPos != null && (this.shouldUseShortPath && CaveNoise.CONFIG.CAN_CLIMB())) {
    ...
    private void resetAttackCooldown() {
        int attackSpeed = Utils.secondsToTicks((int) CaveNoise.CONFIG.ATTACK_RATE());
